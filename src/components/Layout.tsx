...
export function Layout() {
  ...
  return (
    <div className={`flex min-h-screen flex-col bg-slate-100 text-slate-900 lg:flex-row ${isPresentationMode ? 'lg:block' : ''}`.trim()}>
      {!isPresentationMode && (
      ...
      <main className={`${isPresentationMode ? 'min-h-screen p-0' : 'flex-1 p-3 sm:p-4 lg:p-6'}`}>
        {!isPresentationMode && (
          <>
        ...
        {!isPresentationMode && !isAnswerOpen && <AskSigmaBar />}
        <Outlet />
        {!isPresentationMode && <AnswerPanel />}
      </main>
    </div>
  )
}
