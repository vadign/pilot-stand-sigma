...
- `/display` — большой экран. Маршрут сам создаёт presentation session, показывает QR-код и ждёт команды с телефона.
- `/mobile?s=SESSION` — пульт управления. Можно отправлять Ask Sigma, использовать голосовой ввод и переключать ключевые панели мэра.
- Синхронизация идёт через SSE endpoint `/session/{sid}/stream`, а сами команды передаются через `/session/{sid}/ask` и `/session/{sid}/present`.
...
- `/briefing` — legacy-отчет для существующих сценариев presentation/Ask Sigma.
...
- runtime-refresh включён по умолчанию для всех режимов запуска: сначала клиент пробует живой источник, затем откатывается на snapshot/cache/mock;
...
3. Проверить:
   ...
   - `/reports` — executive summary, дельты и drilldown в детали;
   - `/briefing` — legacy briefing-маршрут;
